...
public class ChargedByStationsBonusJob_MonthOnlyTest : BonusTestApi
{
    ...
    private void AddNoiseSession()
    {
        ...
        session3.operation.cpName = Q.Station3;

        var session4 = Q.CreateSession(new DateTime(1990, 1, 1));
        session4.user.chargingClientType = MongoChargingClientType.CompanyEntity;
        session4.operation.cpName = Q.Station3;

        var session5 = Q.CreateSession(Q.TimeExtMoth1.to.UtcDateTime);
        session5.operation.cpName = Q.Station1;
